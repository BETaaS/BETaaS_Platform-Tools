...
/**
 * Used to allow the clients to request info from a GW
 * @author Intecs
 */
@Path("/instanceInfo")
public class InstanceInfoGetter {
	...
	public Response getInstanceInfo() {
		...
		if ((WSDL != null) && (WSDL.length() > 0)) {
			...
			log.loginfo("Sending the instance info request to BETaaS using WSDL: " + WSDL);
        ...
        String infoResult = "";
        try {
        	infoResult = port.getInstanceInfo();	        
	        log.loginfo("The IM returned: " +  infoResult);
...
//	        }
		...
		return Response.ok(infoResult).cacheControl(cc).build();
	}
}
