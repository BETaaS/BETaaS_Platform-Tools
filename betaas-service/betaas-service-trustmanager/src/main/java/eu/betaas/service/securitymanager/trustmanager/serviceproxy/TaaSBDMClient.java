...
public class TaaSBDMClient 
{
	...
	public void storeTrustData (GatewayTrust trustObject)
	{
		/*
	...
	public GatewayTrust getTrustData (String idGateway)
	{
		/*
		TrustManagerService myTrust = new TrustManagerService();
		...
		return new GatewayTrust (idThingService, security, qosFulfillment, dependability, scalability, batteryLoad, dataStability, trustValue);
		*/
		
		return null;
	}
		
	public ArrayList<GatewayTrustData> getThingData (String idThing)
	{
		ArrayList<GatewayTrustData> myResult = new ArrayList<GatewayTrustData>();
		...
		try
		{
			...
			PreparedStatement myQuery = myConn.prepareStatement("SELECT timestamp, battery_cost, memory_status FROM T_THING_DATA WHERE thingID='" + idThing + "'");			
			...
			while(results.next())
			{
				...
				GatewayTrustData currentData = new GatewayTrustData (theTime, battery, memory);
